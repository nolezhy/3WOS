...
class fisvdd:
    ...
    def find_sv(self):
        ...
        for index, new_data in enumerate(self.data[1:]):
            new_data = np.array([new_data])

            score, sim_vec = self.score_fcn(new_data)
            if score > 0:
                self.expand(new_data, sim_vec, (index + 1))

                if min(self.alpha) < 0:
                    backup,back_label = self.shrink()
                    for back_index, each in enumerate(backup):
                        each = np.array([each])
                        score, sim_vec = self.score_fcn(each)
                        if score > 0:
                            self.expand(each, sim_vec, back_label[back_index])

                self.model_update()

            self.obj_val.append(self.score)

        return self.index.squeeze()
    ...
    def expand(self, new_sv, new_sim_vec, index):
    ...
    def shrink(self):
        ...
        while True:
            ...
            label_out = self.index[min_ind]
            backup.append(data_out)
            back_label.append(label_out)
            pInd = np.where(self.alpha > min(self.alpha))
            self.sv = self.sv[pInd]
            self.index = self.index[pInd]
    ...
    def score_fcn(self, new_data):
        ...
        dist_sq = distance.cdist(new_data, self.sv, metric='jaccard')[0]
